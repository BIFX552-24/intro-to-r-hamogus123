...
Explanation: To properly convert `human_age` back to a numeric vector, we need to first convert the factor into a character vector and then to a numeric.
...
```{r}
# create people
people <- data.frame(first_name = "Hamere", last_name = "Mogus", lucky_number = 19)
  
# add a team member
people <- rbind(people,
                data.frame(first_name = "Elias", last_name = "Alamirew", lucky_number = 16))
...
The output of the data.frame we are working with can be explained as the following: the data frame has 1704 observations and 6 variables. country and continent are factors with 142 and 5 levels respectively. year and population are integers. lifeExp and gdpPercap are both numerical.
