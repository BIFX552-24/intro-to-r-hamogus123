...
# 3. The "weight" column has the incorrect data type which is character.
...
```{r}
mean_male_height <- c(age_20_39 = 176.1, age_40_59 = 175.8, age_60_plus = 173.4)
str(mean_male_height)
#The data type of the names of mean_male_height is a character vector.
```
...
This line of code will return the first variable of the cats data set as a data frame. The single brackets will return a data frame with a single column of data.

```{r}
cats[[1]]
```

This line of code will return the first variable of cats as a character vector.

```{cats$coat}
```

This line of code will do the same thing as the double brackets meaning that it will return a character vector.

```{r}
cats["coat"]
```

This line of code will do the same thing as the single brackets meaning that it will return a data frame with a single column of data.

```{r}
cats[1, 1]
```

This line of code will return the first observation of the first column.

```{r}
cats[, 1]
```

This line of code will return a character vector of the first column for all observations.

```{r}
cats[1, ]
```

This line of code will return a data frame that contains all the variables for the first observation.
...
The result of length(matrix_example) is 18, there are a total of 18 values in the matrix.
