...
#The code below will filter it by column 
for (i in 1:5) {
  for (j in 1:5) {
    temp_j_value <- j_vector[j]
    temp_output <- paste(i, temp_j_value)
...
for(i in levels(gapminder$continent))
{
  ...
  if(mean_lifeExp >= 50)
  {
    paste(i, "has a mean life expectancy greater than 50 year") %>%
      print()
  }else{
...
for(i in levels(gapminder$country))
{
  ...
  if(mean_lifeExp < 50)
  {
    paste(i, "has a mean life expectancy less than 50 year") %>%
      print()
  }else if(mean_lifeExp < 70 ){
    paste(i, "has a mean life expectancy between 50 and 70 years") %>%
      print()
  }else{
    paste(i, "has a mean life expectancy greater than 70 year") %>%
      print()
  }
}
```
